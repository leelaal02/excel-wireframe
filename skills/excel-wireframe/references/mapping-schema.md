...
| `meta_table` | `{"tables": [...], "labels": {...}}` | **`layout` 전용.** 상단 메타 표에 값을 채운다. 아래 참고 |
...
화면설계서 상단의 메타 표(`프로젝트명 | 값 | 산출물명 | 값 | …`)를 채운다.

```json
"meta_table": {
  "tables": ["메타표1", "메타표2"],
  "labels": {
    "화면명": "title", "ID": "screen_id", "산출물명": "문서제목",
    "프로젝트명": "프로젝트명", "버전": "버전", "작성일": "작성일"
  }
}
```

- `tables` — 값을 찾을 표. 레이아웃의 표를 위→아래 순으로 이 개수만큼 본다.
  이름은 표를 세는 용도이지 매칭 키가 아니다 — 템플릿마다 표 이름이 제각각이고
  겹치기까지 하기 때문이다(실제 샘플은 표 둘 다 `Group 10`이다).
- `labels` — 키는 표 안의 라벨 문자열, 값은 가져올 이름. 라벨 셀을 찾아 **바로
  오른쪽 칸 자리**에 쓰므로 열 인덱스를 적지 않는다. `title`·`screen_id`는 화면의
  이름·ID를 가리키는 예약어이고, 나머지는 화면별 `fields` → 문서 `meta` 순으로 찾는다.
- 값을 못 찾으면 빈칸으로 두지 않고 **`입력필요`를 빨강(FF0000)·굵게** 쓴다.
  `excel.meta_overrides`로 채우면 그 칸부터 값이 들어간다.
- 이 섹션이 없으면 경로 전체를 건너뛴다. 기존 매핑은 그대로 동작한다.

**레이아웃의 표는 슬라이드로 상속되지 않는다.** PowerPoint는 레이아웃 도형 중
placeholder만 물려주므로, 표는 배경으로 비쳐 보일 뿐 값을 쓸 대상이 없다.

그렇다고 표를 복제해 덮지는 않는다 — 표가 두 겹이 되어 원본을 클릭할 수 없고
테두리가 두 번 그려진다. **원본 표는 그대로 두고, 값이 들어갈 칸 좌표에 글자만
올린다**(`slide_layout.add_meta_text_slots`). 그 자리의 도형 이름은 `메타:{라벨}`
이고(`meta_slot_name`), 글자 크기·굵기·색·정렬은 그 칸이 품고 있던 서식을 그대로
복사한다 — 빈 칸은 그것을 `endParaRPr`에 담고 있다.
...
**본문 영역 높이에 비례해 환산한** 값이고, 4행을 넘으면 마지막 값을 반복한다.
이미지 자리는 표 위의 나머지를 전부 차지하고, 사진은 그 안에서 비율을 지켜 위에 붙는다(가로는 가운데).

**행높이는 내용에 따라 늘리지 않는다.** 늘리면 상세가 긴 화면만 표가 위로 자라
슬라이드마다 사진 크기가 달라진다. 넘치는 글은 글자 크기를 낮춰(7 → 6.5 → 6pt)
맞추고, 6pt로도 안 들어가면 `text-overflow` 경고를 남긴다. 그래서 슬라이드 크기나
레이아웃이 달라져도 모든 화면에서 사진과 표가 같은 자리·같은 크기다.

같은 이유로 긴 스크린샷을 여러 조각으로 나눌 때도 조각마다 가로세로 비가 다르면
안 된다 — `image_split.slice_image`가 짧은 조각 아래에 흰 여백을 덧대 비를 맞춘다
(잘라 내지 않으므로 내용은 그대로다).
...
```json
"template": {
  "file": "output/.work/default-template.pptx",
  "mode": "layout",
  "layout": "화면",
...
| `image_split` | `auto` | 화면 이미지가 PPT 이미지 자리보다 길면 세로로 나눠 여러 장에 싣는다. 자를 곳은 가로 여백 띠(섹션 경계)에서 고르므로 표나 입력폼이 중간에 끊기지 않는다. 각 조각에 든 SoM 뱃지(노란 원) 수만큼 상세를 배분하고, 한 조각의 상세가 슬롯을 넘으면 같은 조각을 여러 장에 반복해 싣는다. 뱃지가 안 잡히거나 개수가 상세 건수와 어긋나면 분할은 하되 상세는 순서대로 배분하고 그 사실을 출력에 남긴다. `off`로 끈다. 기준이 되는 이미지 자리는 `layout` 모드에서는 계산한 값이고, `clone` 모드에서는 예시 슬라이드의 `shapes.image` 앵커 도형이다 — 그 이름이 비었거나 도형을 못 찾으면 분할하지 않는다 |
...
`images`는 작업 폴더(`output/.work/`) 기준 상대 경로다.
...
`build.py`는 그 자리에 빨강·굵게 `입력필요`를 남긴다 — 채워야 할 자리가 있다는
사실이 산출물에서 사라지지 않게 하기 위해서다. 쪽번호처럼 자동 필드(`a:fld`)를
담은 자리만 예외로, 값이 없으면 필드를 그대로 둔다(글자를 쓰면 필드가 사라진다).
화면별 `fields`에 같은 키가 있으면 `fields`가 `meta`보다 우선한다.

## summaries.json

Excel의 상세 설명은 화면설계서에 그대로 싣기엔 길다(실제 샘플 평균 94자). 추출과
생성 사이에서 Claude가 화면설계서 어투로 줄여 `output/.work/summaries.json`에 쓰면
`build.py`가 `options.detail_text_source`가 가리키는 값을 이것으로 갈아끼운다.

```json
{ "CPTIPMT1002": { "1": "[저장] 기본정보 저장 · 필수·유효성 검사 후 저장, 재조회" } }
```

화면ID → 상세번호(`no`) → 요약문. 번호는 Excel 값 그대로다(스크린샷의 SoM 뱃지와
대응하므로 재부여하지 않는다). 파일이 없거나 어떤 항목이 빠져 있으면 그 항목은
원문을 그대로 쓴다. `extract.py`는 이 파일을 덮어쓰지 않으므로 재추출해도 요약이
남는다 — 사람이 직접 고쳐도 된다.
