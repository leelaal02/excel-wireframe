...
| `image_split` | `auto` | 화면 이미지가 PPT 이미지 자리보다 길면 세로로 나눠 여러 장에 싣는다. 자를 곳은 가로 여백 띠(섹션 경계)에서 고르므로 표나 입력폼이 중간에 끊기지 않는다. 각 조각에 든 SoM 뱃지(노란 원) 수만큼 상세를 배분하고, 한 조각의 상세가 슬롯을 넘으면 같은 조각을 여러 장에 반복해 싣는다. 뱃지가 안 잡히거나 개수가 상세 건수와 어긋나면 분할은 하되 상세는 순서대로 배분하고 그 사실을 출력에 남긴다. `off`로 끈다. **`layout` 모드 전용이다** |
