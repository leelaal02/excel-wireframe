...
def _setup(tmp_path: Path):
    ...
    return xlsx, out_dir, mp


def test_extract_reads_mapping_from_work_dir(tmp_path: Path):
    """--mapping을 생략하면 .work/mapping.json을 읽는다."""
    xlsx = make_sheet_per_screen_xlsx(tmp_path / "s.xlsx", SPEC)
    out_dir = tmp_path / "output"
    ...
    data = read_json(out_dir / ".work" / "screens.json")
    assert data["screens"][0]["id"] == "SCR001"


def test_extract_keeps_output_dir_free_of_work_files(tmp_path: Path):
    """결과물 폴더 루트에는 아무것도 남기지 않는다. pptx만 놓일 자리다."""
    xlsx = make_sheet_per_screen_xlsx(tmp_path / "s.xlsx", SPEC)
    out_dir = tmp_path / "output"
    write_json(out_dir / ".work" / "mapping.json", MAPPING)
    ...
    assert (out_dir / ".work" / "images" / "SCR001.png").exists()
    assert not (out_dir / "screens.json").exists()
    assert not (out_dir / "images").exists()
    assert [p.name for p in out_dir.iterdir()] == [".work"]


def test_extract_migrates_legacy_files(tmp_path: Path):
    """구버전 폴더로 돌리면 .work/로 옮기고 그대로 이어서 추출한다."""
    xlsx = make_sheet_per_screen_xlsx(tmp_path / "s.xlsx", SPEC)
    out_dir = tmp_path / "output"
    ...
    assert read_json(out_dir / ".work" / "screens.json")["screens"][0]["id"] == "SCR001"


def test_extract_creates_screens_json(tmp_path: Path):
    xlsx, out_dir, mp = _setup(tmp_path)
    code = main(["--excel", str(xlsx), "--mapping", str(mp), "--output", str(out_dir)])
    assert code == 0
    data = read_json(out_dir / ".work" / "screens.json")
    assert data["screens"][0]["id"] == "SCR001"
...
def test_extract_overwrites_existing_screens_json(tmp_path: Path):
    ...
    write_json(out_dir / ".work" / "screens.json", {"meta": {}, "screens": [
        {"id": "OLD", "name": "옛 화면", "images": [], "fields": {}, "details": []}
    ]})

    assert main(["--excel", str(xlsx), "--mapping", str(mp), "--output", str(out_dir)]) == 0

    data = read_json(out_dir / ".work" / "screens.json")
    assert [s["id"] for s in data["screens"]] == ["SCR001"]
    assert not (out_dir / "screens.new.json").exists()
...
def test_extract_puts_cover_meta_into_screens_json(tmp_path: Path):
    ...
    assert main(["--excel", str(xlsx), "--mapping", str(mp), "--output", str(out_dir)]) == 0
    meta = read_json(out_dir / ".work" / "screens.json")["meta"]
...
def test_extract_no_longer_hardcodes_title(tmp_path: Path):
    xlsx, out_dir, mp = _setup(tmp_path)
    main(["--excel", str(xlsx), "--mapping", str(mp), "--output", str(out_dir)])
    meta = read_json(out_dir / ".work" / "screens.json")["meta"]
